...
variables {
  ...
  proxy_interfaces       = "place-holder"
  dkim_keys_prefix       = "place-holder"
  force_pull             = "place-holder"
  mynetworks             = "place-holder"
  relay_domains          = "place-holder"
  transport              = "place-holder"
  resource_count         = 0
}

job "mailserver-in" {
  name        = "${var.nomad_env}-mailserver-in"
  region      = "global"
  ...
  group "mailserver-in" {
    ...
    volume "mailserver_in_queue" {
      type            = "csi"
      source          = "mailserver_in_queue"
    ...
    task "mailserver-in" {
      ...
      env {
        ...
        POSTFIX_MYORIGIN                      = "${var.url_prefix_env}mail.example.org"
        POSTFIX_PROXY_INTERFACES              = var.proxy_interfaces
        POSTFIX_MYNETWORKS                    = "0.0.0.0/0"
        POSTFIX_MYDOMAIN                      = "example.org"
        POSTFIX_MYHOSTNAME                    = "${var.url_prefix_env}mail.example.org"
        POSTFIX_MAIL_NAME                     = "${var.url_prefix_env}mail.example.org"
        POSTFIX_RELAY_DOMAINS                 = var.relay_domains
      ...
      dynamic "service" {
        ...
        content {
          name = "${var.nomad_env}-${service.value}-in"
          port = service.value
          tags = ["_tcp"]
          check {
            type     = "tcp"
      ...
      template { # trigger a restart when the version changes on Consul
        destination = "local/deployment_trigger"
        data        = <<EOF
version: {{ key "nomad/${var.nomad_env}/mailserver/docker_postfix_version" }}
...
EOF
      ...
      template {
        uid         = 999
        gid         = 100
        destination = "local/postfix/transport.hash"
        data        = <<EOF
{{ key "nomad/${var.nomad_env}/mailserver/transport.hash" }}
EOF
      }
      template {
        perms       = "0600"
...
EOF
      ...
      config {
        ...
        mount {
          type     = "bind"
          source   = "./local/postfix/transport.hash"
          target   = "/etc/postfix/tables/transport.hash"
          readonly = true
        }
      }
